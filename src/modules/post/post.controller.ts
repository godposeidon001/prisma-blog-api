...
const buildPostFilters = (
...
): { ok: true; filters: PostQueryFilters } | { ok: false; error: string } => {
  const search =
  ...
  const authorIdValue = forcedAuthorId ?? getSingleQueryValue(req.query.authorId);
...
const getAllPosts = async (req: Request, res: Response) => {
  try {
    const result = buildPostFilters(req);
    if (!result.ok) {
    ...
    const posts = await postService.getAllPosts(result.filters);
...
const getMyPosts = async (req: Request, res: Response) => {
  try {
    ...
    const result = buildPostFilters(req, req.user.id);
    if (!result.ok) {
      return res.status(400).json({
        error: result.error,
      });
